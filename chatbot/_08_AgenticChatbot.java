...
public interface AgendaPlanner {
  ...
  TokenStream createAgenda(@V("talks") String talks, @V("request") String request);
}

/// The Devoxx Agenda Planner: a sequential + loop agentic workflow.
...
void main() {
  ...
  // === Non-streaming model (for TalkSearcher and QualityScorer) ===
  // java-54
  ...
  // === Streaming model (for AgendaPlanner — last agent in sequence) ===
  // java-55
  ...
  // === Build the loop: QualityScorer → TalkSearcher (refinement) ===
  // Loops until score >= 0.7 or max 3 iterations
  // java-57
  UntypedAgent qualityLoop = AgenticServices
      .loopBuilder()
      .subAgents(talkSearcher, qualityScorer)
      .maxIterations(3)
      .exitCondition(scope -> scope.readState("score", 0.0) >= 0.7)
      .build();

  // === Build Agent 3: AgendaPlanner (streaming) ===
  // java-58
  ...
  // Run the agentic workflow via the interactive menu -> assistant.chat(prompt)
  // java-60
